...
try {
    ...
    $recipe_check_stmt = $communityDB->prepare("SELECT user_id FROM RecipeDB.recipes WHERE recipe_id = ?");
    ...
    if ($user_id && $user_id != $author_id) {
        ...
        if (!isset($_SESSION[$session_key])) {
            ...
            $count_stmt = $communityDB->prepare("SELECT COALESCE(view_count, 0) AS view_count FROM RecipeDB.recipes WHERE recipe_id = ?");
